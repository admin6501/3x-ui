...
// derivePublicKey returns the base64-encoded Curve25519 public key paired
// with the supplied base64-encoded secret key. WireGuard uses Curve25519 so
// the public key is computed by performing a scalar-multiplication of the
// secret key against the curve's basepoint. Mirrors the JS-side
// `Wireguard.generateKeypair(secretKey).publicKey` helper, which only runs
// on the panel UI and never persists pubKey to settings JSON.
func derivePublicKey(secretKeyB64 string) (string, error) {
        sk, err := base64.StdEncoding.DecodeString(secretKeyB64)
        if err != nil {
                return "", err
        }
        if len(sk) != curve25519.ScalarSize {
                return "", fmt.Errorf("wireguard secret key must be %d bytes, got %d", curve25519.ScalarSize, len(sk))
        }
...
func (s *SubService) genWireguardLink(inbound *model.Inbound, peerIndex int) string {
        ...
        if pubKey == "" {
                // The panel's JS layer regenerates pubKey on every load and never
                // persists it, so the JSON pulled from DB has no pubKey field.
                // Derive it locally from the (always-stored) secretKey so the
                // generated wireguard:// link contains the peer's public key.
                if sk, ok := settings["secretKey"].(string); ok && sk != "" {
                        if derived, err := derivePublicKey(sk); err == nil {
                                pubKey = derived
                        }
                }
        }

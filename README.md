...
- **Reseller sales bot** — a second Telegram bot (its own token, its own admin list, switched on from Panel Settings → Telegram → Sales Bot) that sells reseller panels in Persian. Define packages in the panel or in the bot — price, traffic quota, client slots, which inbounds; buyers browse them, upload a payment receipt, and once you approve it the panel creates their reseller account and sends them the credentials. Buying again tops the same account up instead of issuing a second login, and a top-up brings back an account suspended for running out of quota. The bot also carries an admin side: pending orders, package management, reseller list with enable/disable and traffic reset, sales figures and a broadcast. The **Sales** page mirrors all of it, and can record a sale made outside Telegram.

...
// SumInboundClientTraffic returns the sum of up+down across every client
// traffic row attached to the given inbound. Used by the reseller quota
// accounting on bulk-reset operations: we need to bill the soon-to-be-zeroed
// bytes against the reseller BEFORE the reset wipes them. Pass id == -1 to
// sum across *all* inbounds (mirrors ResetAllClientTraffics' semantics).
//
// Returns 0 on any DB error so a transient hiccup never silently undercharges
// quota — callers can rely on the next reset to catch up if needed.
func (s *InboundService) SumInboundClientTraffic(id int) int64 {
        db := database.GetDB()
        var row struct{ Total int64 }
        q := db.Model(xray.ClientTraffic{}).Select("COALESCE(SUM(up + down), 0) AS total")
        if id == -1 {
                q = q.Where("inbound_id > ?", id)
        } else {
                q = q.Where("inbound_id = ?", id)
        }
